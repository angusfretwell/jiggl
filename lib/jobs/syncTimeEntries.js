...
export default async function (_, done) {
  try {
    const promises = [];
    const since = moment().subtract(1, 'hours').toISOString();
    ...
    for (const entry of body.data) {
      ...
      if (process.env.APP_DEBUG && entry.project.indexOf('TEST') < 0) {
        winston.info(`Skipping ${entry.id} in debug mode`);
        continue;
      }
    ...
    winston.info('Sucessfully schronised worklog');
    return done();
  } catch (err) {
    winston.error('Failed to synchronise worklog', err);
    return done(err);
  }
}
